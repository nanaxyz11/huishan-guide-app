...
if "followup_questions" not in st.session_state:
    st.session_state.followup_questions = []      # 当前轮生成的后续问题
...
def generate_followup_questions(user_question, ai_answer):
    ...
    FOLLOWUP_API_KEY = "Bearer app-CCck7NxI8NLZIxf24Q247Hti"   # ⚠️ 必须替换为第二步创建的Key

    prompt = f"""用户问题：{user_question}
AI 回答：{ai_answer}
请根据以上内容，生成 3 个与文化遗产相关的、开放式的、引导用户继续深入探索的后续问题。每个问题一行，只输出三个问题，不要序号，不要额外解释。"""
    ...
    try:
        ...
        # 去除可能的序号
        import re
        questions = [re.sub(r'^\d+[\.\、]?\s*', '', line) for line in lines[:3]]
        while len(questions) < 3:
            questions.append("您还想了解这个景点的更多信息吗？")
        return questions
    except Exception:
        # 降级默认问题
        return [
...
# 只对 free_text 条件实现完整对话历史+后续问题生成
